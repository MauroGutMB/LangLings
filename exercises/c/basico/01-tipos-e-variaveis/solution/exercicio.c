...
/* bytes_de_doubles devolve o espaço ocupado por `quantidade` doubles.
 *
 * O (long) vem antes da multiplicação porque sizeof devolve size_t e a
 * quantidade é int: sem ele, a conta aconteceria na largura menor e só o
 * resultado já truncado seria promovido.
 */
long bytes_de_doubles(int quantidade)
{
    return (long)quantidade * (long)sizeof(double);
}
