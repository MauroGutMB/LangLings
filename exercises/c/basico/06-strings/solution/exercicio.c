...
/* comeca_com devolve 1 quando s começa com prefixo.
 *
 * strncmp compara só os n primeiros caracteres, então se prefixo for maior
 * que s, ela para no '\0' de s e já devolve diferente de zero — não é preciso
 * comparar comprimentos antes.
 */
int comeca_com(const char *s, const char *prefixo)
{
    return strncmp(s, prefixo, strlen(prefixo)) == 0;
}
