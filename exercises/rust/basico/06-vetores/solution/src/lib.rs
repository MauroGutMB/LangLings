...
// pares devolve um Vec novo só com os números pares de ns, na mesma ordem.
//
// `copied()` transforma o `&i32` que `filter` produz de volta em `i32`, para
// o Vec resultante ser `Vec<i32>` como a assinatura promete, não `Vec<&i32>`.
pub fn pares(ns: &[i32]) -> Vec<i32> {
    ns.iter().filter(|n| **n % 2 == 0).copied().collect()
}
