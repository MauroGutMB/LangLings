...
// contar_palavras conta quantas vezes cada palavra de texto aparece.
//
// split_whitespace já ignora espaços repetidos e extremidades, então uma
// string vazia (ou só de espaços) devolve um iterador vazio e o mapa some
// vazio — sem caso especial para tratar.
pub fn contar_palavras(texto: &str) -> HashMap<String, u32> {
    let mut contagem = HashMap::new();
    for palavra in texto.split_whitespace() {
        *contagem.entry(palavra.to_string()).or_insert(0) += 1;
    }
    contagem
}
