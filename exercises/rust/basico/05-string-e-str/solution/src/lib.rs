...
// gritar devolve uma String nova com s em maiúsculas seguido de "!".
//
// to_uppercase() já aloca uma String; push_str reaproveita essa alocação em
// vez de criar mais uma com `+`.
pub fn gritar(s: &str) -> String {
    let mut resultado = s.to_uppercase();
    resultado.push_str("!");
    resultado
}
