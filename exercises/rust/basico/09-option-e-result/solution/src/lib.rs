...
// somar_textos converte a e b para i32 e devolve a soma.
//
// Os dois `?` propagam o primeiro erro que aparecer: se `a` já falhar, `b`
// nem chega a ser convertido.
pub fn somar_textos(a: &str, b: &str) -> Result<i32, String> {
    let x: i32 = a.parse().map_err(|_| format!("'{a}' não é um número"))?;
    let y: i32 = b.parse().map_err(|_| format!("'{b}' não é um número"))?;
    Ok(x + y)
}
