...
//
// Cada `if` é uma expressão: o maior entre a e b vira um valor intermediário,
// e esse valor é comparado com c na mesma forma — nenhum `return` precisa
// interromper o fluxo em ponto nenhum.
pub fn maior_de_tres(a: i32, b: i32, c: i32) -> i32 {
    let maior_dos_dois = if a > b { a } else { b };
    if maior_dos_dois > c { maior_dos_dois } else { c }
}
