...
// Resumir devolve o valor total por categoria.
//
// Os dois erros saem antes de qualquer soma: um resumo parcial junto de um
// erro seria pior do que nenhum, porque quem chama tenderia a usá-lo.
func Resumir(itens []Item) (map[string]int, error) {
	if len(itens) == 0 {
		return nil, fmt.Errorf("resumir estoque: %w", ErrSemItens)
	}

	out := make(map[string]int, len(itens))
	for _, it := range itens {
		if it.Categoria == "" {
			return nil, fmt.Errorf("resumir estoque: item %q: %w", it.Nome, ErrCategoriaVazia)
		}
		out[it.Categoria] += it.Valor()
	}
	return out, nil
}
